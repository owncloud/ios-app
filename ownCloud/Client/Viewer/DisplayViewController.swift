		...
		if isUpdate {
			if allowUpdatesUntilLocalModificationPersists {
				...
				switch updateStrategy {
					case .ask:
						OnMainThread {
							let alert = UIAlertController(title: String(format: "%@ was updated".localized, item.name ?? "File".localized), message: "Would you like to view the updated version?".localized, preferredStyle: .alert)
