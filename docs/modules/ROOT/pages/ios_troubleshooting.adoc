...
You will find on the same screen where you enable logging the the access to the log files. Touch menu:Share log file[] which opens a new screen with all the log files created. To export one selected, tap the share button on the top right-hand side of the screen.
