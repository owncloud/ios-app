...
|bool
|`true`
|Controls whether the app should prompt for an App Store review.
|advanced `candidate`
...
|stringArray
|
|The identifiers of the authentication methods allowed for this profile. Allows to f.ex. force OAuth2, or to use Basic Auth even if OAuth2 is available. See connection.allowed-authentication-methods for possible values.
|advanced `candidate`
